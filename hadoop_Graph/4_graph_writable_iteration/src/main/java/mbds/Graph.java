...
public class Graph {
    ...
    // Compteur des noeuds qui n'ont pas ete parcourus
	public static enum GRAPH_COUNTERS {
		NB_NODES_UNFINISHED			    
	};
	...
	public static class GraphReduce extends Reducer<Text, GraphNodeWritable, Text, GraphNodeWritable>{
		...
		public void reduce(Text key, Iterable<GraphNodeWritable> values, Context context) throws IOException, InterruptedException {	
			...
			if(!node.color.equals("NOIR")) {
				context.getCounter(GRAPH_COUNTERS.NB_NODES_UNFINISHED).increment(1);	            
			}
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
	    ...
	    int iteration = 0;
		String base_output_path = ourArgs[1]; // ex. "graph-output"

		while(true){
			// Obtient un nouvel objet Job: une tache Hadoop. On fourni la configuration Hadoop ainsi qu'une description textuelle de la tache.
			...
			String suffix = "-" + Integer.toString(iteration); // ex. "-0" // ex. "-1" ...
			if (iteration == 0){ // ex. 0 // 1 ...
				// A la premiere iteration notre fichier / dossier d'entre correspond au premier argument
				FileInputFormat.addInputPath(job, new Path(ourArgs[0]));
			} else {
				// A tout prochaine iteration on lit ce qu'on a ecrit a la dernier iteration
				FileInputFormat.addInputPath(job, new Path(base_output_path + suffix)); // ex. "graph-output-1"
				            
			}
			iteration++; // ex. 1 // ex. 2 ...
			suffix = "-" + Integer.toString(iteration); // ex. "-1" // ex. "-2"
			FileOutputFormat.setOutputPath(job, new Path(base_output_path + suffix)); // ex.  "graph-output-1"

			// On lance la tache Hadoop.
			if(!job.waitForCompletion(true)) {
				// Si la tache hadoop echoue on renvoie -1
				System.exit(1);
			}

			// La tache  s'est effectuee correctement.
			Counters cn = job.getCounters();
			Counter cnt = cn.findCounter(GRAPH_COUNTERS.NB_NODES_UNFINISHED);
			// Si tous les noeuds ont ete traite, on renvoie 0
			if(cnt.getValue() == 0) {
